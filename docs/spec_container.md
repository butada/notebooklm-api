...
- 初期認証投入用に `/bootstrap/nlm-auth.tar.gz` を read-only でマウント可能とする。

## 4. 起動方針
- 認証状態が空の場合のみ、必要に応じてアーカイブから復元する。
...
services:
  nlm-api:
    ...
    environment:
      ...
      - NLM_AUTH_ARCHIVE=/bootstrap/nlm-auth.tar.gz
      - NLM_FORCE_RESTORE_AUTH=0
    volumes:
      - ./secrets/nlm-auth.tar.gz:/bootstrap/nlm-auth.tar.gz:ro
